 ...
 public:
  ...
  void writeMessagepackInteger(UInt value) {
    ...
    if (isSigned)  // signed values
    {
      ...
      for (int8_t i = intSize - 1, state = 0; i >= 0;
           i--)  // search for empty bytes or full negative bytes
      {
        switch (state) {
          case 0:
    ...
    } else  // unsigned values
    {
      ...
      {
        if (convertInt.bytes[i] == 0)
          intSize--;  // count effective size
        else
          break;
      }
 ...
 private:
  ...
  static inline uint8_t roundSize(uint8_t intSize) {
    ...
    if (intSize > sizeof(uint16_t)) {
      ...
      {
        intSize -=
            (intSize < sizeof(uint32_t) ? sizeof(uint32_t) - intM : intM);
