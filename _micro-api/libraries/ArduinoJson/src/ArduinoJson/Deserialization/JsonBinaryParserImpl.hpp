...
template <typename TReader, typename TWriter>
inline bool ArduinoJson::Internals::JsonBinaryParser<
    TReader, TWriter>::parseAnythingToUnsafe(JsonVariant *destination) {
  uint8_t value = _reader.current();

  if (value == MESSAGEPACK_ARRAY_16 || (value >= MESSAGEPACK_ARRAY_FIX_MIN &&
                                        value <= MESSAGEPACK_ARRAY_FIX_MAX))
    return parseArrayTo(destination);
  else if (value == MESSAGEPACK_MAP_16 || (value >= MESSAGEPACK_MAP_FIX_MIN &&
                                           value <= MESSAGEPACK_MAP_FIX_MAX))
...
ArduinoJson::Internals::JsonBinaryParser<TReader, TWriter>::parseArray() {
  ...
  if (arrayType >= MESSAGEPACK_ARRAY_FIX_MIN &&
      arrayType <= MESSAGEPACK_ARRAY_FIX_MAX) {
...
template <typename TReader, typename TWriter>
inline bool
ArduinoJson::Internals::JsonBinaryParser<TReader, TWriter>::parseArrayTo(
    JsonVariant *destination) {
...
ArduinoJson::Internals::JsonBinaryParser<TReader, TWriter>::parseObject() {
  ...
  if (objectType >= MESSAGEPACK_MAP_FIX_MIN &&
      objectType <= MESSAGEPACK_MAP_FIX_MAX) {
...
template <typename TReader, typename TWriter>
inline bool
ArduinoJson::Internals::JsonBinaryParser<TReader, TWriter>::parseObjectTo(
    JsonVariant *destination) {
...
ArduinoJson::Internals::JsonBinaryParser<TReader, TWriter>::parseString() {
  ...
  if (stringType >= MESSAGEPACK_STRING_FIX_MIN &&
      stringType <= MESSAGEPACK_STRING_FIX_MAX) {
    strSize = stringType - MESSAGEPACK_STRING_FIX_MIN;
    _reader.move();
    if (strSize == 0) return NULL;
  } else if (((int8_t)stringType >= MESSAGEPACK_INT_S5_MIN) &&
             ((int8_t)stringType <= MESSAGEPACK_INT_U7_MAX)) {
    strSize = 0;
    ...
    switch (stringType) {
      ...
      MSGPACK_MOUNT_STR:
        ...
        while (strSize) {
          strSize--;
          if (isString == 1)
            convertSize.bytes[strSize] = _reader.current();
          else
...
template <typename TReader, typename TWriter>
inline bool
ArduinoJson::Internals::JsonBinaryParser<TReader, TWriter>::parseStringTo(
    JsonVariant *destination) {
