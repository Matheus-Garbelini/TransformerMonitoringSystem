...
typedef struct {
  uint32_t v;
} float32_t;  // used for soft convertion
typedef struct {
  uint64_t v;
} float64_t;  // used for soft convertion
struct commonNaN {
  bool sign;
  uint64_t v0, v64;
};
...
static uint32_t softfloat_shiftRightJam32(uint32_t a, uint_fast16_t count) {
  return (count < 31) ? a >> count | ((uint32_t)(a << (-count & 31)) != 0)
                      : (a != 0);
}
...
static uint64_t softfloat_shortShiftRightJam64(uint64_t a, uint_fast8_t count) {
